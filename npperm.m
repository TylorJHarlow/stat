function [truT, z, p, cv] = npperm(A, B, iter, correction)
    ...
    if nargin < 4 || isempty(correction)
      correction = 1;
    end
    N = length(A);
    ...
    for j = 1:iter
        for i = 1:N
            ...
            subjectcondA(i,:,:) = squeeze(nanmean(subjectAll(idx,:,:)));
            subjectcondB(i,:,:) = squeeze(nanmean(subjectAll(idx2,:,:)));
            if j ==1
                truA(i,:,:) = squeeze(nanmean(A{i},1));
                truB(i,:,:) = squeeze(nanmean(B{i},1));
            else
                break
            end
        end
    ...
    z = squeeze((truT - nanmean(tstat))./std(tstat));  p = ((1-normcdf(abs(z)))*2);
